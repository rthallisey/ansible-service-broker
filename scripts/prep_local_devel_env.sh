...
###
TLS_CRT_DATA=`oc get secret asb-tls -o jsonpath='{ .data.tls\.crt }' -n ansible-service-broker`
...
# Fetch the tls.crt for the asb deployment
###
TLS_KEY_DATA=`oc get secret asb-tls -o jsonpath='{ .data.tls\.key }' -n ansible-service-broker`
# Remove quotes from variable
